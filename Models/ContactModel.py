...
class ContactModel:
    ...
    def create_contact(self, blind_id, name, relation, age, gender, created_at):
        cursor = self.db.execute("""
            ...
            OUTPUT INSERTED.id
            VALUES (?, ?, ?, ?, ?, ?)
        """, (blind_id, name, relation, age, gender, created_at))
        
        new_id = cursor.fetchone()[0]  # <<< fetch first
        cursor.close()                  # <<< then close
        self.db.commit()                # <<< then commit
        return new_id
    ...
    def get_contacts_by_user(self, blind_id):
        return self.db.execute(
            """
            SELECT c.id, c.name, c.relation, c.age, c.gender, c.created_at, cp.pic_url
            FROM Contact c
            LEFT JOIN ContactPics cp ON c.id = cp.contact_id
            WHERE c.blind_id=?
            ORDER BY c.name
            """,
            (blind_id,)
